...
#
# Handles the solving part of the analytical solution
# =============================================================================
def analytical():
    d_value = 1 / (3 * sigma_t)

    analytical_sln = numpy.zeros([ints + 1, 1])
    index = 1

    alpha = math.sqrt(sigma_a / d_value)
    qterm = q_value / (4 * pi * d_value)

    epos = math.exp(alpha * radius)
    bterm = epos / radius - 2 * d_value * (epos / math.pow(radius, 2) - alpha * epos / radius)

    eneg = math.exp(alpha * radius * -1)
    aterm = eneg / radius - 2 * d_value * (alpha * eneg / radius + eneg / math.pow(radius, 2))

    c1 = qterm * (bterm / (bterm - aterm))
    c2 = qterm * (aterm / (bterm - aterm))
...
# =============================================================================
def plot_solutions():
  marker_style_1 = dict(color='crimson', linestyle=':', marker='_', markersize=4)
  marker_style_2 = dict(color='teal', linestyle=':', marker='+', markersize=4)

  pyplot.figure(0)
  pyplot.clf()

  analyticalSolution = analytical()
  asol2 = analytical2()

  matrix_a = []
  vector_b = []

  nfld2 = numerical(d_value)
  trez1 = calc_residual(nfld2)
  tbal1 = calc_balance(nfld2, trez1, 1)
  print('\n----------------------------------------------')
  print('Non-Limited:')
  print("Total Residual: ", trez1)
  print("Total Balance:   ", tbal1)
  print('----------------------------------------------\n')

  fsol1 = fld_solution()
  trez2 = calc_residual(fsol1)
  tbal2 = calc_balance(fsol1, trez2, 2)
  print('\n----------------------------------------------')
  print('Flux-Limited:')
  print("Total Residual:  ", trez2)
  print("Total Balance:   ", tbal2)
  print('----------------------------------------------\n')

  n = numpy.size(fsol1) - 1
  phi_diff = numpy.zeros(n)
  for i in range(0, ints):
      phi_diff[i] = abs(analyticalSolution[i] - fsol1[i])

  numpy.savetxt('phidiff.txt', phi_diff, fmt="%4.6f")
  #print(phi_diff)

  r1, jlimited = calc_jvalue_limited(st, dr, fsol1)
  r2, jnonlimted = calc_jvalue_nonlimited(st, dr, nfld2)
